...
import type { EngagementRewardGrant, EngagementSignInStatusResponse, EngagementStreakFreezeStatus } from '../types/paper'
...
// C2: surface load failures so the UI can show a retry affordance
const loadError = ref(false)
// C4: persistent streak-break flag shown in the panel banner (session-scoped)
const streakBroken = ref(false)

// Track reward codes earned this session for NEW indicator on the progress bar
const newRewardCodes = ref(new Set<string>())

// Active rewards per feature (lazy-loaded when a feature panel opens)
...
// ---------------------------------------------------------------------------
// Shared reward utility functions (exported for component reuse — P5)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Milestone reward preview — maps streak_day to reward summary for progress bar
// ---------------------------------------------------------------------------

export interface MilestoneRewardPreview {
  name: string
  code: string
  /** One-line benefit text shown in the next-milestone hint */
  brief: string
}
...
// ---------------------------------------------------------------------------
// Reward usage hints — maps reward_code to where/how to use it (P0 / P4)
// ---------------------------------------------------------------------------

export interface RewardUsageHint {
  /** Short description shown in toast / achievements page */
  text: string
  /** Router path to navigate to for direct-use rewards */
  route?: string
  /** Label for the navigation CTA button */
  routeLabel?: string
}

export const REWARD_USAGE_HINTS: Record<string, RewardUsageHint> = {
  day2_chat_boost_ticket: {
    text: '打开任意论文详情页或通用助手，在对话框附近开启此券。下次对话 AI 可读取 1.5 倍的论文上下文，回答更深入',
    route: '/',
    routeLabel: '去发起对话',
  },
  day3_fast_track_ticket: {
    text: '下次上传论文时，在上传对话框中打开此券——论文会跳过普通队列优先处理，省去等待',
    route: '/my-papers',
    routeLabel: '去上传论文',
  },
  day4_idea_boost_ticket: {
    text: '打开灵感生成面板，在生成前开启此券。AI 将从 1.5 倍的知识原子池中提取灵感，生成更多元、更有深度的研究方向',
    route: '/',
    routeLabel: '去生成灵感',
  },
  day5_compare_plus_ticket: {
    text: '打开日报或灵感页面，选中多篇论文发起对比，对比面板会显示此券。开启后在当前套餐基础上额外 +2 篇对比名额',
    route: '/',
    routeLabel: '去发起对比',
  },
  day7_research_accelerator: {
    text: '发起深度研究时，在研究面板顶部开启此券。AI 获得 1.5 倍上下文额度和最多 30 篇精选范围，分析结果更完整',
    route: '/',
    routeLabel: '去发起深度研究',
  },
  day30_compare_premium_ticket: {
    text: '发起论文对比时，在对比面板中使用此券，在当前套餐基础上额外 +4 篇对比名额——适合做系统性文献综述',
    route: '/',
    routeLabel: '去发起对比',
  },
  day60_research_premium_ticket: {
    text: '发起深度研究或对比时，在对应面板中开启此券：2 倍上下文 + 最多 30 篇精选 + 额外 +4 篇对比名额，一券全覆盖',
    route: '/',
    routeLabel: '去发起研究或对比',
  },
}
...
export interface EngagementToastItem {
  ...
  streak?: number
  /** C1: which task was just completed (for task_done type) */
  ...
  /** For reward_expiring / reward_used: contextual message */
  message?: string
  /** For completion toast: next upcoming milestone info */
...
// Track which reward IDs have already received an expiry toast this session
...
export function useEngagement() {
  ...
  const streakText = computed(() => {
    ...
    return next
      ? `连续有效研究日：${s.current}（下个里程碑：第 ${next} 天）`
      : `连续有效研究日：${s.current}`
  })
    ...
    return [
      {
        ...
        label: '浏览',
        hint: '打开任意论文详情或 PDF',
        done: tasks?.view ?? false,
      },
      {
        key: 'collect',
        label: '收藏',
        hint: '右滑或点击收藏论文',
        done: tasks?.collect ?? false,
      },
      {
        key: 'analyze',
        label: '分析',
        hint: '向 AI 提问（免费每日 10 次）、发起对比或深度研究',
        done: tasks?.analyze ?? false,
      },
    ]
  })
  ...
  async function loadStatus(force = false) {
    ...
    try {
      ...
      // C4: set panel-level streak-break flag once per session (persistent banner)
      if (!_streakBreakNotified) {
        ...
        if (streak && streak.current === 0 && streak.longest > 0) {
          ...
          // Also push a toast so the user is notified even without opening the panel
          toastQueue.value.push({ id: ++_toastId, type: 'streak_break' })
        }
      }

      // P1: Notify once per session for active, non-honorary rewards expiring within 3 days
      ...
      for (const r of rewards) {
        ...
        if (diffDays <= 3 && diffDays >= 0) {
          ...
          break // At most one expiry toast per loadStatus call to avoid notification spam
        }
      }
    } catch {
      // C2: surface the error so the UI can show a retry affordance
  ...
  async function record(action: 'view' | 'collect' | 'analyze', source: string, targetId?: string) {
    try {
      // C1: snapshot task state before the request to detect first-time flips
      const prevTasks = status.value?.progress?.tasks
      const wasAlreadyDone = prevTasks?.[action] ?? false

      const next = await recordEngagementTask({
        action,
        source,
        target_id: targetId,
      })
      ...
      // C1: if this action just flipped from false → true and the day isn't yet fully
      // completed (completion toast handles that case), show a lightweight task-done chip
      ...
      // Completion feedback (covers the last-task flip as well)
      if (next.just_completed) {
        ...
        toastQueue.value.push({
          ...
          nextMilestone: nextMsPreview && nextMsDay
            ? { day: nextMsDay, name: nextMsPreview.name, brief: nextMsPreview.brief }
            : undefined,
        })
      }

      // Milestone reward notification (shown after completion toast)
      if (next.just_granted_reward) {
        const r = next.just_granted_reward
        // Mark this reward code as newly earned for the NEW indicator
      ...
      // Pro trial notification (bridge: engagement → entitlement)
      if ((next as any).just_granted_pro_trial) {
        const trial = (next as any).just_granted_pro_trial as {
          message: string
          trial_days: number
        ...
        // Refresh entitlements so Pro features unlock immediately
        try {
          const entitlements = useEntitlements()
          await entitlements.refreshEntitlements(true)
        } catch { /* entitlements refresh is best-effort */ }
      }
  ...
  // C4: let the panel banner dismiss itself once the user acknowledges it
  function dismissStreakBroken() {
    streakBroken.value = false
  }

  // Clear NEW indicator once user has seen their rewards
  function markRewardsRead() {
    newRewardCodes.value = new Set()
  }

  // Check if a specific reward code is newly earned (for NEW badge on reward rows)
  function isNewReward(code: string): boolean {
    return newRewardCodes.value.has(code)
  }

  // Whether any new reward is waiting to be seen
  const hasNewRewards = computed(() => newRewardCodes.value.size > 0)

  // Push a toast confirming that a reward was successfully applied to an operation
  ...
  // Streak freeze state derived from loaded status
  const freezeStatus = computed<EngagementStreakFreezeStatus | null>(
    () => status.value?.freeze ?? null,
  )
  ...
  async function loadActiveRewards(feature: 'chat' | 'idea_gen' | 'compare' | 'research' | 'upload') {
    try {
      ...
      // Silently ignore — reward loading failure should not block core feature
    }
  }
  ...
  const bestChatReward = computed<EngagementRewardGrant | null>(() => {
    ...
    return rewards.reduce((best, r) => {
      const bMult = (best.boost?.input_hard_limit_multiplier ?? 1)
      const rMult = (r.boost?.input_hard_limit_multiplier ?? 1)
      return rMult > bMult ? r : best
    })
  })

  const bestIdeaGenReward = computed<EngagementRewardGrant | null>(() => {
    const rewards = activeRewardsIdeaGen.value.filter(r => r.status === 'active')
    return rewards[0] ?? null
  })

  const bestCompareReward = computed<EngagementRewardGrant | null>(() => {
    // Return the reward that gives the highest compare items delta
    const rewards = activeRewardsCompare.value.filter(r => r.status === 'active')
    if (!rewards.length) return null
    return rewards.reduce((best, r) => {
      const bDelta = (best.boost?.compare_items_delta ?? 0)
      const rDelta = (r.boost?.compare_items_delta ?? 0)
      return rDelta > bDelta ? r : best
    })
  })
  ...
  const bestUploadReward = computed<EngagementRewardGrant | null>(() => {
    const rewards = activeRewardsUpload.value.filter(r => r.status === 'active')
    return rewards[0] ?? null
  })

  // P7: Highest honorary badge earned by the user, for Navbar display
  const highestBadge = computed<{ code: string; name: string; emoji: string } | null>(() => {
    ...
    const emojiMap: Record<string, string> = {
      day100_legend_badge: '🌟',
      day14_researcher_badge: '🎓',
      day1_focus_badge: '🌱',
    }
    for (const code of badgeOrder) {
      const r = rewards.find(r => r.reward_code === code)
      if (r) {
        return { code, name: r.reward_name, emoji: emojiMap[code] ?? '🏆' }
      }
    }
    return null
  })
  ...
  return {
    ...
    record,
    dismissToast,
    dismissStreakBroken, // C4
    // NEW reward indicator
    hasNewRewards,
    isNewReward,
    markRewardsRead,
    notifyRewardUsed,
    // Reward feature
    activeRewardsChat,
    activeRewardsIdeaGen,
    activeRewardsCompare,
    activeRewardsResearch,
    activeRewardsUpload,
    bestChatReward,
    bestIdeaGenReward,
    bestCompareReward,
