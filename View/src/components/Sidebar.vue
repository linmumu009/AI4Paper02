...
import type { KbTree, KbFolder, KbPaper, KbMenuItem, KbNote, KbCompareResult, KbCompareResultsTree, KbCompareFolder, UserPaper, UserPaperTree, UserPaperFolder, CompareCartItem, UserPaperFileViewMode, UserPaperViewMdPayload } from '../types/paper'
...
// ---- Tab switching ----
type SidebarTab = 'papers' | 'compare' | 'mypapers' | 'paper-inspiration'
...
const emit = defineEmits<{
  ...
  openUploadDialog: []
  tabChanged: [tab: 'papers' | 'compare' | 'mypapers' | 'paper-inspiration']
  /** 论文灵感：生成指定论文的灵感候选卡片 */
...
defineExpose({ refreshAllExpandedNotes, updateNoteTitle, refreshMyPapers, switchToMyPapersTab, loadMyPapers, onPaperInspirationDone })
</script>
...
<template>
    ...
    <div class="px-4 pt-5 pb-4 border-b border-border">
      ...
      <!-- Hero section: daily highlight -->
      <div class="mb-4">
        <div class="text-[10px] font-semibold tracking-[0.2em] uppercase text-text-muted/60 select-none mb-1">Daily Digest</div>
        <div class="flex items-center justify-between gap-2">
          <div class="text-[24px] font-bold leading-none tracking-tight text-text-primary">每日精选</div>
          <div class="flex items-center gap-1.5 shrink-0">
            <span
              v-if="totalPaperCount > 0"
              class="inline-flex items-center gap-1 text-[11px] text-text-muted"
            >
            ...
            <span v-else class="text-[11px] text-text-muted/60">AI 前沿</span>
            <!-- Collapse button -->
            <button
              class="w-10 h-10 flex items-center justify-center rounded-md text-text-muted/50 hover:text-text-primary hover:bg-bg-elevated transition-colors cursor-pointer shrink-0"
              title="收起侧边栏"
      ...
      <div class="flex items-stretch gap-0.5 p-1 bg-bg-elevated/80 rounded-xl border border-border/40">
        <!-- 知识库 -->
        <button
          class="relative flex-1 flex flex-col items-center justify-center gap-1 py-2 px-1 rounded-lg text-[12px] font-semibold transition-all duration-200 cursor-pointer border-none"
          :class="activeTab === 'papers'
            ? 'bg-bg-card text-text-primary shadow-sm'
            : 'bg-transparent text-text-muted hover:text-text-secondary'"
          @click="activeTab = 'papers'; selectFolder(null)"
        >
          <svg class="w-3.5 h-3.5 shrink-0 transition-colors duration-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
            <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
          </svg>
        ...
        >
          ...
          <span class="leading-none">对比库</span>
        </button>
        ...
        >
          ...
          <span class="leading-none">我的论文</span>
        </button>

        <!-- 论文灵感 (paper-inspiration mode) -->
        <button
        ...
        >
          ...
          <span class="leading-none">论文灵感</span>
        </button>
      </div>

      <!-- Toolbar row (papers tab) -->
      <div v-if="activeTab === 'papers'" class="flex items-center justify-end gap-2 mt-2.5">
        <button
          class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-transparent border cursor-pointer transition-colors"
          :class="batchMode
        ...
        <button
          class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-transparent border border-border text-text-muted hover:text-tinder-pink hover:border-tinder-pink/40 cursor-pointer transition-colors"
          title="新建文件夹"
      ...
      <!-- Toolbar row (mypapers tab) -->
      <div v-if="activeTab === 'mypapers'" class="flex items-center justify-end gap-2 mt-2.5">
        <button
          class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-transparent border cursor-pointer transition-colors"
          :class="batchMode
            ? 'border-tinder-pink/60 text-tinder-pink bg-tinder-pink/5'
        ...
        <button
          class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-transparent border border-border text-text-muted hover:text-tinder-pink hover:border-tinder-pink/40 cursor-pointer transition-colors"
          title="新建文件夹"
          @click="startMyPaperNewFolder(myPaperActiveFolderId)"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/>
          </svg>
        ...
        >
          ...
          <div v-if="expandedPapers.has(paper.paper_id)" class="pb-1">
            ...
            <div
              ...
              style="padding-left: 50px; padding-right: 8px;"
            >
              <div class="flex flex-1 min-w-0 items-center gap-2 cursor-pointer" @click="emit('viewMd', { paperId: paper.paper_id, title: (paper.paper_data.short_title || paper.paper_id) + ' · 中文翻译', pdfUrl: paper.pdf_static_url ? `${API_ORIGIN}${paper.pdf_static_url}` : null, mdUrl: `${API_ORIGIN}${paper.zh_static_url}`, viewMode: 'zh', mineruUrl: paper.mineru_static_url ? `${API_ORIGIN}${paper.mineru_static_url}` : null, zhUrl: paper.zh_static_url ? `${API_ORIGIN}${paper.zh_static_url}` : null, bilingualUrl: paper.bilingual_static_url ? `${API_ORIGIN}${paper.bilingual_static_url}` : null })">
                <span class="text-xs shrink-0">🇨🇳</span>
            ...
            <div
              ...
              style="padding-left: 50px; padding-right: 8px;"
            >
              <div class="flex flex-1 min-w-0 items-center gap-2 cursor-pointer" @click="emit('viewMd', { paperId: paper.paper_id, title: (paper.paper_data.short_title || paper.paper_id) + ' · 中英对照', pdfUrl: paper.pdf_static_url ? `${API_ORIGIN}${paper.pdf_static_url}` : null, mdUrl: `${API_ORIGIN}${paper.bilingual_static_url}`, viewMode: 'bilingual', mineruUrl: paper.mineru_static_url ? `${API_ORIGIN}${paper.mineru_static_url}` : null, zhUrl: paper.zh_static_url ? `${API_ORIGIN}${paper.zh_static_url}` : null, bilingualUrl: paper.bilingual_static_url ? `${API_ORIGIN}${paper.bilingual_static_url}` : null })">
                <span class="text-xs shrink-0">🔀</span>
    ...
    <div v-if="activeTab === 'mypapers'" class="flex-1 overflow-y-auto flex flex-col">
      ...
      <div class="px-3 pt-2 pb-1 shrink-0">
        <button
          class="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-white bg-gradient-to-r from-[#f59e0b] to-[#ef4444] border-none cursor-pointer hover:opacity-90 transition-opacity"
      ...
      <div v-else class="flex-1 overflow-y-auto p-2" @click="myPaperActiveFolderId = null">
        ...
        <div v-if="!myPaperTree || (myPaperTree.folders.length === 0 && myPaperTree.papers.length === 0)" class="text-center py-8">
          <div class="w-16 h-16 mx-auto mb-3 rounded-xl bg-gradient-to-br from-[#f59e0b] to-[#ef4444] opacity-60 flex items-center justify-center">
