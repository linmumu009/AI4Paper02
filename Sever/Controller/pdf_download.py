...
def resolve_json_path(args_json: Optional[str]) -> str:
    ...
    # Prefer RUN_DATE env (set by the scheduler / app.py) so the correct
    # date's dedup file is picked up even when the clock has advanced past
    # midnight or sits in a different timezone from the scheduler.
    _run_date_env = os.environ.get("RUN_DATE", "").strip()
    if _run_date_env:
        today = _run_date_env + ".json"
    else:
        today = datetime.now().strftime(JSON_FILENAME_FMT)
    # Priority order for shared pipeline (multi-user mode):
    #   1. paperList_remove_duplications -- ALL deduped papers (before theme filter)
    #   2. paper_theme_filter            -- legacy: already-filtered papers
    #   3. llm_select_theme              -- fallback with scores
    #   4. raw arxiv json                -- last resort
    for base_dir in [PAPER_DEDUP_DIR, PAPER_THEME_FILTER_DIR, LLM_SELECT_THEME_DIR, ARXIV_JSON_DIR]:
    ...
    raise FileNotFoundError(
        f"No json found in {PAPER_DEDUP_DIR}, {PAPER_THEME_FILTER_DIR}, "
        f"{LLM_SELECT_THEME_DIR} or {ARXIV_JSON_DIR}"
    )
