...
def resolve_json_path(explicit: str | None) -> str:
    ...
    # date's manifest is picked up even when the clock has advanced past
    # midnight or sits in a different timezone from the scheduler.
    _run_date_env = os.environ.get("RUN_DATE", "").strip()
    if _run_date_env:
        # First check if a manifest already exists in raw_pdf/<date>
        dated_manifest = os.path.join(PDF_OUTPUT_DIR, _run_date_env, MANIFEST_FILENAME)
        if os.path.isfile(dated_manifest):
            return dated_manifest
        # Otherwise fall back to json files for that date
        for base_dir in [PAPER_THEME_FILTER_DIR, LLM_SELECT_THEME_DIR, ARXIV_JSON_DIR]:
            candidate = os.path.join(base_dir, _run_date_env + ".json")
