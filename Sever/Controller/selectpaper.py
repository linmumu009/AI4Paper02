...
.. deprecated::
    ...
    In the new multi-user pipeline the paper-selection step is performed by
    ``paper_theme_filter.py`` and ``instutions_filter.py`` writing directly to
    the pipeline DB (``--output-mode db``).  The per_user pipeline no longer
    includes this step.
