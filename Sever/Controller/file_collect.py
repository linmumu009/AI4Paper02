...
===============
.. deprecated::
    This controller is retained ONLY for backward-compatibility with the legacy
    single-user pipeline (``default`` / ``daily`` pipelines in app.py).

    In the new multi-user pipeline, paper data is read directly from the
    pipeline DB (``pipeline_db_service``) via ``data_service.py``.  The
    ``file_collect`` directory tree is no longer the primary data source.

    ``data_service.list_dates`` / ``get_papers_by_date`` fall back to
    ``data/file_collect/`` automatically when the DB has no rows for a date,
    ensuring backward compatibility for existing deployments.

    For any new development, use the DB-aware pipeline instead.
"""
