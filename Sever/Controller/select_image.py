...
def run() -> None:
    ...
    # Also persist per-paper image filenames to DB so the summary JSON can later be removed
    try:
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).parent.parent))
        from services.pipeline_db_service import bulk_upsert_paper_images
        images_map: dict[str, list[str]] = {}
        for report in reports:
            stem = report.get("stem")
            if not stem or report.get("error"):
                continue
            out_dir_path = Path(report.get("output_dir", ""))
            if out_dir_path.is_dir():
                imgs = sorted(
                    f.name for f in out_dir_path.iterdir()
        ...
        if images_map:
            ...
            print(f"[SELECT_IMAGE] saved image index to DB for {len(images_map)} papers", flush=True)
    except Exception as _db_err:
        print(f"[SELECT_IMAGE] WARNING: failed to write image index to DB: {_db_err!r}", flush=True)
