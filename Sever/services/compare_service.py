...
def stream_compare(
    ...
    if not has_llm_credentials(cfg):
        ...
        _admin_llm = _uss.resolve_admin_llm_for_feature(feature)
    ...
    if not has_llm_credentials(cfg):
        ...
        yield f"data: {json.dumps(f'请先在「个人中心 → {feature_label}」中配置 LLM 的 API Key 和 Model，或选择一个模型预设。', ensure_ascii=False)}\n\n"
        yield "data: [DONE]\n\n"
        return
