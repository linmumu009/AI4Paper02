...
def user_has_candidate_access(user_id: int, candidate_id: int) -> bool:
    """Check if a user may read a candidate they don't own.

    Two cases grant access:
    1. The user has any feedback record for this candidate (they saw it in digest).
    2. The candidate originates from a shared pipeline (not a user-created draft),
       meaning it was generated by the admin/system pipeline and is safe to expose
       to all authenticated viewers.
    """
    conn = _connect()
    try:
        fb = conn.execute(
            "SELECT 1 FROM idea_feedback WHERE user_id = ? AND candidate_id = ? LIMIT 1",
            (user_id, candidate_id),
        ).fetchone()
        if fb:
            return True
        cand = conn.execute(
            "SELECT source_type FROM idea_candidates WHERE id = ?",
            (candidate_id,),
        ).fetchone()
        if cand and cand["source_type"] in ("pipeline", "paper_inspiration", None):
            return True
        return False
    finally:
        conn.close()
