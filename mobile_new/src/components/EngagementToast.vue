...
import {
  ...
  isHonoraryReward,
  rewardIcon,
  type EngagementToastItem,
} from '@shared/composables/useEngagement'
...
function navigateAndDismiss(id: number, webPath: string) {
  dismissToast(id)
  router.push(toMobilePath(webPath))
}
...
function onTouchEnd(e: TouchEvent, id: number) {
  ...
  if (dx < -48) dismissToast(id)
}
</script>

<template>
  <div
    class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[80] flex flex-col items-center gap-2 pointer-events-none max-w-[min(90vw,340px)] w-full px-2"
    aria-live="polite"
  >
    <TransitionGroup
    ...
    >
      <div
        ...
        :key="toast.id"
        class="pointer-events-auto w-full rounded-2xl shadow-2xl overflow-hidden bg-bg-card border touch-pan-y"
        :class="toastWrapClass(toast.type)"
        @touchstart.passive="onTouchStart($event, toast.id)"
        @touchend.passive="onTouchEnd($event, toast.id)"
      >
        <template v-if="toast.type === 'task_done'">
          <div class="px-3.5 py-2.5 flex items-center gap-2.5">
            ...
            <button type="button" class="shrink-0 text-text-muted hover:text-text-primary transition-colors bg-transparent border-none cursor-pointer p-0.5" @click="dismissToast(toast.id)">
        ...
        <template v-else-if="toast.type === 'completion'">
          <div class="px-4 py-3 flex items-start gap-3">
            ...
            <button type="button" class="ml-auto shrink-0 text-text-muted hover:text-text-primary transition-colors bg-transparent border-none cursor-pointer p-0.5" @click="dismissToast(toast.id)">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
            </button>
          </div>
        </template>
        ...
        <template v-else-if="toast.type === 'milestone'">
          <div class="h-1 w-full bg-gradient-to-r from-tinder-gold via-tinder-pink to-tinder-gold opacity-90" />
          <div class="px-4 py-3">
            ...
            <template v-if="toast.reward && !isHonoraryReward(toast.reward.reward_code) && REWARD_USAGE_HINTS[toast.reward.reward_code]">
              ...
              >
                {{ REWARD_USAGE_HINTS[toast.reward.reward_code].routeLabel ?? '前往使用' }}
              </button>
            </template>
          </div>
        </template>
