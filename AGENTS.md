...
After completing and validating user-requested file changes, add their absolute paths to `changed_files_abs_paths.txt`, keeping the list deduplicated, then run `upload_changed_files.ps1` to synchronize them to the configured server unless the user explicitly says not to sync. Upload only source, configuration, and intentional artifact files; never add private keys, `.env` files, credentials, runtime data, `node_modules`, test results, or generated build directories. The sync script uses the repository-external private key `%USERPROFILE%\.ssh\ai4papers_sync_ed25519`; do not copy that key into this repository or to the server.
