...
- Daily digest file (core deliverable): a single, ready-to-read report that aggregates summaries of the day’s selected papers (for quick daily scanning).
...
- **MinerU Token** (`minerU_Token`): used for PDF → Markdown via MinerU. Create at `https://mineru.net/apiManage/token`.
...
| `theme_select_system_prompt`  | `llm_select_theme.py`  | System prompt for topic relevance scoring (0–1 score)         |
...
| `pdf_info_system_prompt`      | `pdf_info.py`          | Rules for institution detection + “is_large” + short abstract |
...
| `--page-size`     | `PAGE_SIZE_DEFAULT`   | Page size (1–2000)                                                         |
...
├── 📄 app.py                          # Main entry: orchestrates Controller steps by pipeline
├── 📄 pdf_download.log                # Log file of pdf_download.py
├── 📄 readmePrinceple.md              # Notes & principles for writing the README
├── 📂 Controller/                     # Core pipeline step scripts
│  ├── 📂 __pycache__/                 # Python bytecode cache
│  ├── 📄 arxiv_search04.py            # Step 1: arXiv fetch (query + time window)
│  ├── 📄 llm_select_theme.py          # Step 2: LLM topic-relevance scoring
│  ├── 📄 paper_theme_filter.py        # Step 3: filter by relevance score
│  ├── 📄 http_session.py              # Shared requests Session + retry logic
│  ├── 📄 instutions_filter.py         # Step 6: filter “large-institution” papers
│  ├── 📄 paperList_remove_duplications.py # Step 1.1: de-duplicate & track processed papers
│  ├── 📄 paper_summary.py             # Step 11: generate Chinese summaries from full MinerU md
│  ├── 📄 pdf_download.py              # Step 4: download raw PDFs (per date subdir)
│  ├── 📄 pdf_info.py                  # Step 7: LLM-based institution recognition & abstract
│  ├── 📄 pdf_split.py                 # Step 5: cut preview PDFs (first N pages)
│  ├── 📄 pdfsplite_to_minerU.py       # Step 6: preview PDFs → MinerU Markdown
│  ├── 📄 selectedpaper_to_mineru.py   # Step 10: selected PDFs → full MinerU Markdown
│  ├── 📄 selectpaper.py               # Step 9: move “large institution” PDFs to selected dir
│  ├── 📄 summary_limit.py             # Step 11.5: compress summaries into fixed structure
│  ├── 📄 zotero_push.py               # Step 12: push selected papers into Zotero
├── 📂 config/                         # Central configuration
│  ├── 📂 __pycache__/                 # Bytecode cache
│  ├── 📄 config copy.py               # Old backup of config (for reference only)
│  ├── 📄 paperList.json               # Global list of processed papers (for de-dup)
├── 📂 data/                           # Runtime data (organized by date)
│  ├── 📂 arxivList/                   # Daily candidate lists (md/json)
│  │  ├── 📂 md/                       # Candidate Markdown lists
│  │  └── 📂 json/                     # Candidate JSON lists
│  ├── 📂 paperList_remove_duplications/ # De-duplicated candidate JSON
│  ├── 📂 llm_select_theme/            # JSON after LLM topic scoring
│  ├── 📂 paper_theme_filter/          # JSON after topic filtering
│  ├── 📂 raw_pdf/                     # Raw PDFs + manifest
│  ├── 📂 preview_pdf/                 # Preview PDFs + manifest
│  ├── 📂 preview_pdf_to_mineru/       # MinerU md + manifest for previews
│  ├── 📂 pdf_info/                    # Institution recognition JSON
│  ├── 📂 instutions_filter/           # “large institution” paper lists
│  ├── 📂 selectedpaper/               # Selected PDFs + manifest
│  ├── 📂 selectedpaper_to_mineru/     # Full MinerU md + manifest for selected PDFs
│  ├── 📂 paper_summary/               # Summaries (single + daily gather)
│  ├── 📂 summary_limit/               # Compressed summaries (single + daily gather)
│  ├── 📂 select_image/                # Summary image pages (PNG) + report
│  └── 📂 file_collect/                # Final collected packages by paper id
├── 📂 logs/                           # Runtime logs (by date)
└── 📂 reference/                      # Reference projects & sample code (copied from old repo)
```
...
- 7) Preview PDFs → MinerU Markdown (`pdfsplite_to_minerU.py`)
- 8) Institution detection & structured info (`pdf_info.py`)
- 9) Build “large institution” PDF list (`instutions_filter.py`)
- 10) Move selected PDFs (`selectpaper.py`)
- 11) Full selected PDFs → MinerU Markdown (`selectedpaper_to_mineru.py`)
...
- **Output**: sequential execution of step scripts (`Controller/*.py`, see steps 1–16).
...
- **Logic**
  ...
  - For each paper in the daily candidate list:
    - If key exists in history → consider “already processed”, skip
    - Else → append `{title, source, writing_datetime}` to history
  - Write a new JSON keeping only “not-seen-before” papers, preserving meta

> If you want later steps to only work on “new” papers, pass  
...
- **Logic**
  ...
  - Call LLM in parallel to get a 0–1 relevance score
...
### 7) Preview PDF → MinerU Markdown (`Controller/pdfsplite_to_minerU.py`)
...
- **Logic**
  - MinerU batch flow: request upload URL → PUT upload → poll result → download zip → extract md
...
### 9) Build “large institution” PDF list (`Controller/instutions_filter.py`)
...
### 11) Selected PDF → full MinerU Markdown (`Controller/selectedpaper_to_mineru.py`)
...
- **Logic**
  - If total length ≤ 950 (non-whitespace chars), copy as-is
  ...
  - Enforce bullet style (`🔸`) and item counts for each section

---

### 14) Summary image pages (`Controller/select_image.py`)

- **Goal**: render page 1 of the PDF as a cover, then select “result-related” figures from MinerU output and layout multi-page PNGs with the same size as the cover.
- **Input**
  - Selected PDFs (`data/selectedpaper/<date>/<arxiv_id>.pdf`)
...
- **Logic**
  ...
  - Select “result-related” figures based on content list and keyword matching
  - Layout as multi-page PNGs using configured engine (HTML/CSS or ReportLab)
  - Generate a statistics report
- **Run examples**

```bash
# Option A: HTML/CSS layout → Chromium print to PDF → render to PNG (more stable)
...
```bash
# Option B: Justified rows layout → ReportLab → PNG
